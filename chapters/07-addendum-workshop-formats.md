...
Six movable walls or similar are required, on which the six theses (printed out in DIN A1 format as shown in the above image) can be attached. Sticky dots and sticky notes in two colors (green and yellow) serve as working materials. The complete template I usually use is available as a [PowerPoint template in German and English on my homepage](https://raitner.de/en/2019/04/the-workshop-on-the-manifesto-for-humane-leadership/) and can be used freely.

## Workshop Format for Small Groups {#workshop-format-for-small-groups}
...
With 1-2-4-All, large groups can work on a common question: each participant alone for one minute, then in pairs for two minutes, and again in groups of four for four minutes. In the end, each group of four briefly presents its most outstanding idea or insight to everyone. The exact process of 1-2-4-All is also described in the book by Henri Lipmanowicz and Keith McCandless.

The questions to be addressed are based on the three scaling questions described at the beginning. After briefly introducing the respective thesis, everyone writes their answers on paper. Three minutes should be set aside for this. The insights gained are then discussed and condensed, first in groups of two and then in groups of four. At the end, the highlights of each group of four are shared with everyone.
